...
parser.add_argument('--model-path', type=str, default=r'utils\aim-csgo2.pt', help='模型地址')
parser.add_argument('--imgsz', type=int, default=640, help='和你训练模型时imgsz一样')
parser.add_argument('--conf-thres', type=float, default=0.3, help='置信阈值')
...
parser.add_argument('--lock-sen', type=float, default=1, help='lock幅度系数；若在桌面试用请调成1，在游戏中(csgo)则为灵敏度')
parser.add_argument('--lock-smooth', type=float, default=1.5, help='lock平滑系数；越大越平滑，最低1.0')
...
parser.add_argument('--lock-tag', type=list, default=[1, 0], help='对应标签；缺一不可，自己按以下顺序对应标签，ct_head ct_body t_head t_body')
parser.add_argument('--lock-choice', type=list, default=[0, 1], help='目标选择；可自行决定锁定的目标，从自己的标签中选')
...
parser.add_argument('--recoil-button-m4a1', type=str, default='x1', help='m4a1压枪按键；同上')
